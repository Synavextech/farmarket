 ...
 */
export const generatePdfReceipt = async (depositData: any): Promise<string> => {
    return new Promise((resolve, reject) => {
        try {
            ...
            const fileName = `receipt-${depositData.id || uuidv4()}.pdf`;
 ...
 */
export const generateAggregatePdfReport = async (data: any[], meta: any): Promise<string> => {
    return new Promise((resolve, reject) => {
        try {
            const doc = new PDFDocument({
                margin: 50,
            ...
            const fileName = `report-${meta.ref || uuidv4()}.pdf`;
            const uploadDir = path.join(process.cwd(), 'uploads');
            if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });
            ...
            data.forEach((row, index) => {
                ...
                let cellX = 50;
                doc.text(row.user_name || row.users?.full_name || 'N/A', cellX + 5, currentY + 5, { width: columns[0].width - 10, ellipsis: true });
                cellX += columns[0].width;
                doc.text(row.national_id || row.users?.national_id || 'N/A', cellX + 5, currentY + 5);
                ...
                doc.text(row.phone_number || row.users?.phone_number || 'N/A', cellX + 5, currentY + 5);
